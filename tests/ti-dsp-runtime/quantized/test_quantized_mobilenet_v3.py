"""
Quantized MobileNetV3 Large DSP test.

Tests INT8 quantized MobileNetV3 Large on DSP comparing against PyTorch reference.

Usage:
    pytest test_quantized_mobilenet_v3.py -v --dsp-mode=c7x_host
    pytest test_quantized_mobilenet_v3.py -v --dsp-mode=c7x_host --mmalib
    pytest test_quantized_mobilenet_v3.py -v --dsp-mode=c7x_dload
    python test_quantized_mobilenet_v3.py --dsp-mode c7x_host
"""
...
from model_utils import create_quantized_mobilenet_v3_model
...
def _run_test(
    ...
    tvm_mod, quantized_gm, input_data = create_quantized_mobilenet_v3_model()
...
def test_quantized_mobilenet_v3_dsp(
    ...
    record_cycles("mobilenet_v3_int8", results["dsp_results"].get("c7x_dload_cycles", 0))
    assert_dsp_comparison(results["dsp_results"], results["comparison"])


def main():
    parser = argparse.ArgumentParser(description="Quantized MobileNetV3 Large DSP Test")
    ...
    print(f"Quantized MobileNetV3 Large DSP Test (mode: {args.dsp_mode})")
    results = _run_test(
