"""
Quantized InceptionV3 DSP test.

Tests INT8 quantized InceptionV3 on DSP comparing against PyTorch reference.
InceptionV3 requires 299x299 input.

Usage:
    pytest test_quantized_inception_v3.py -v --dsp-mode=c7x_host
    pytest test_quantized_inception_v3.py -v --dsp-mode=c7x_host --mmalib
    pytest test_quantized_inception_v3.py -v --dsp-mode=c7x_dload
    python test_quantized_inception_v3.py --dsp-mode c7x_host
"""
...
from model_utils import create_quantized_inception_v3_model
...
def _run_test(
    ...
    tvm_mod, quantized_gm, input_data = create_quantized_inception_v3_model()
...
def test_quantized_inception_v3_dsp(
    ...
    record_cycles("inception_v3_int8", results["dsp_results"].get("c7x_dload_cycles", 0))
    assert_dsp_comparison(results["dsp_results"], results["comparison"])


def main():
    parser = argparse.ArgumentParser(description="Quantized InceptionV3 DSP Test")
    ...
    print(f"Quantized InceptionV3 DSP Test (mode: {args.dsp_mode})")
    results = _run_test(
