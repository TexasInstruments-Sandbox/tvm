"""
Quantized GoogLeNet DSP test.

Tests INT8 quantized GoogLeNet on DSP comparing against PyTorch reference.

Usage:
    pytest test_quantized_googlenet.py -v --dsp-mode=c7x_host
    pytest test_quantized_googlenet.py -v --dsp-mode=c7x_host --mmalib
    pytest test_quantized_googlenet.py -v --dsp-mode=c7x_dload
    python test_quantized_googlenet.py --dsp-mode c7x_host
"""
...
from model_utils import create_quantized_googlenet_model
...
def _run_test(
    ...
    tvm_mod, quantized_gm, input_data = create_quantized_googlenet_model()
...
def test_quantized_googlenet_dsp(
    ...
    record_cycles("googlenet_int8", results["dsp_results"].get("c7x_dload_cycles", 0))
    assert_dsp_comparison(results["dsp_results"], results["comparison"])


def main():
    parser = argparse.ArgumentParser(description="Quantized GoogLeNet DSP Test")
    ...
    print(f"Quantized GoogLeNet DSP Test (mode: {args.dsp_mode})")
    results = _run_test(
