...
Tests CLISTA-DoA (Convolutional LISTA for Direction of Arrival) model on DSP
(host emulation and/or C66x hardware) comparing against PyTorch reference.

CLISTA-DoA processes complex I/Q radar signals and produces a sparse angular
spectrum representation for direction of arrival estimation.

Usage:
    # Run with C66x host emulation
    pytest test_clista_dsp.py -v

    # Run on C66x hardware
    pytest test_clista_dsp.py -v --dsp-mode=c66x

    # Run via DLOAD pipeline (c7x_compute)
    pytest test_clista_dsp.py -v --dsp-mode=c7x_dload

    # Run as standalone script
    python test_clista_dsp.py --dsp-mode c66x_host
    python test_clista_dsp.py --dsp-mode c66x
    python test_clista_dsp.py --dsp-mode c7x_dload
"""
...
from model_utils import create_clista_model  # noqa: E402  # pyright: ignore[reportMissingImports]
...
def _run_clista_dsp_test(
    ...
    """
    Run CLISTA-DoA model on DSP and compare with PyTorch reference.
    ...
    """
    # Create model
    tvm_mod, torch_model, input_data = create_clista_model()
    ...
    # Compare results
    # C7x uses approximate intrinsics (__recip, __recip_sqrt) that provide
    # ~23-bit mantissa precision vs IEEE float32's 24-bit. Errors accumulate
    # across iterative models like CLISTA, so we use relaxed tolerances.
    if dsp_mode in ("c7x_host", "c7x_dload"):
...
def main():
    """Run as standalone script."""
    parser = argparse.ArgumentParser(description="CLISTA-DoA DSP Test")
    parser.add_argument(
        "--dsp-mode",
        default=None,
    ...
    # Create model
    print("\n[1/3] Creating CLISTA-DoA model...")
    tvm_mod, torch_model, input_data = create_clista_model()

    total_params = sum(p.numel() for p in torch_model.parameters())
    print(f"  Model parameters: {total_params:,}")
    print(f"  Input shape: {input_data.shape}")
    print("  Input: [batch, 2 (I/Q), num_antennas]")
    print("  Output: [batch, num_atoms*2, 1]")
    ...
    # Build target string with optional flags
    target_string = get_target_string(dsp_mode, profile_layers=args.profile_layers, use_cpp_api=args.use_cpp_api)
    if args.use_cpp_api:
    ...
    if "c7x_dload_result" in dsp_results:
        ...
        c7x_dload_passed = np.allclose(dsp_results["c7x_dload_result"], torch_result, rtol=1e-2, atol=2e-2)
