...
class TestInferenceWheelContents:
    ...
    @_skip_source_tree
    def test_deploy_helper_bundled(self):
        """The on-board deploy helper (python3 -m tvm.data.ti_dsp.deploy)
        must ship in the data directory alongside the firmware it installs.
        """
        from tvm.data.ti_dsp.paths import get_ti_dsp_path

        assert (get_ti_dsp_path() / "deploy.py").exists(), "deploy helper missing"
    ...
    def test_compute_binary_path_resolution(self):
        """find_c7x_compute_binary returns path only on aarch64."""
        from tvm.data.ti_dsp.paths import find_c7x_compute_binary

        result = find_c7x_compute_binary()
        if _IS_AARCH64:
            assert result is not None
        else:
            assert result is None
...
"""
        ...
        result = subprocess.run(
            ["ssh", "-o", "ConnectTimeout=10", remote, f"python3 -c {shlex.quote(script)}"],
            capture_output=True,
            text=True,
            timeout=300,
        )
