#!/usr/bin/env python
"""
Conv2D Stack DSP test.

Tests a 4-layer conv2d + batch_norm + relu stack on DSP (host emulation,
C7x host emulation, and/or DLOAD) comparing against PyTorch reference.
The model isolates the most expensive conv2d configurations from ResNet18
without skip connections.

Usage:
    # Run with C66x host emulation
    pytest test_conv2d_stack_dsp.py -v

    # Run with C7x host emulation
    pytest test_conv2d_stack_dsp.py -v --dsp-mode=c7x_host

    # Run via DLOAD pipeline (c7x_compute)
    pytest test_conv2d_stack_dsp.py -v --dsp-mode=c7x_dload

    # Run as standalone script
    python test_conv2d_stack_dsp.py --dsp-mode c66x_host
    python test_conv2d_stack_dsp.py --dsp-mode c7x_host
    python test_conv2d_stack_dsp.py --dsp-mode c7x_dload
"""
...
from model_utils import create_conv2d_stack_model  # noqa: E402
...
def _run_conv2d_stack_dsp_test(
    dsp_mode: str,
    ...
    """
    Run Conv2D stack model on DSP and compare with PyTorch reference.
    ...
    """
    # Create model
    tvm_mod, torch_model, input_data = create_conv2d_stack_model()
...
def main():
    """Run as standalone script."""
    parser = argparse.ArgumentParser(description="Conv2D Stack DSP Test")
    parser.add_argument(
        "--dsp-mode",
        default=None,
    ...
    # Create model
    print("\n[1/3] Creating Conv2D stack model...")
    tvm_mod, torch_model, input_data = create_conv2d_stack_model()
    ...
    if "c7x_host_result" in dsp_results:
        ...
        print(f"[C7x Host] Output range: [{c7x_host_result.min():.6f}, {c7x_host_result.max():.6f}]")
    ...
    if "c7x_dload_result" in dsp_results:
        ...
        # Extract inference cycles from stdout (JSON format from c7x_compute run)
        if "c7x_dload_stdout" in dsp_results:
            stdout = dsp_results["c7x_dload_stdout"]
            cycles_match = re.search(r'"cycles"\s*:\s*(\d+)', stdout)
            if cycles_match:
                cycles = int(cycles_match.group(1))
    ...
    if "c7x_host_result" in dsp_results:
        ...
        c7x_host_passed = np.allclose(
            dsp_results["c7x_host_result"], torch_result, rtol=1e-4, atol=1e-5
        )
    ...
    if "c7x_dload_result" in dsp_results:
        ...
        # Relaxed tolerance for DLOAD due to C7x intrinsic precision
        c7x_dload_passed = np.allclose(dsp_results["c7x_dload_result"], torch_result, rtol=1e-2, atol=1e-2)
