...
See [ARM Host Client Internals](#arm-host-client-internals) for the buffer
...
#### Per-buffer dmabufs (Mechanics and API)
...
runtime. The firmware exports 119 symbols to loaded modules (TVM runtime
...
- The DSP-side service loop runs in the main task context, not a
  dedicated task -- spawning it as a separate task hits a
  `TaskSupport_setupTaskStack` assertion on C7x

## Related Documentation

- [Firmware Design Deep-Dive](design-deep-dive.md) — System architecture, protocol specification, memory layout, DLOAD internals
- [Deploying Firmware](../../user-guide/deploying-firmware.md) — Build, deploy, and usage instructions
- [Python / C++ API Reference](../../user-guide/python-api.md) — Public API surface
