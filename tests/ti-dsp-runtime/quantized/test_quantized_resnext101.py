"""
Quantized ResNeXt-101 (32x8D) DSP test.

Tests INT8 quantized ResNeXt-101 on DSP comparing against PyTorch reference.

Usage:
    pytest test_quantized_resnext101.py -v --dsp-mode=c7x_host
    pytest test_quantized_resnext101.py -v --dsp-mode=c7x_host --mmalib
    pytest test_quantized_resnext101.py -v --dsp-mode=c7x_dload
    python test_quantized_resnext101.py --dsp-mode c7x_host
"""
...
from model_utils import create_quantized_resnext101_model
...
def _run_test(
    ...
    tvm_mod, quantized_gm, input_data = create_quantized_resnext101_model()
...
def test_quantized_resnext101_dsp(
    ...
    record_cycles("resnext101_int8", results["dsp_results"].get("c7x_dload_cycles", 0))
    assert_dsp_comparison(results["dsp_results"], results["comparison"])


def main():
    parser = argparse.ArgumentParser(description="Quantized ResNeXt-101 (32x8D) DSP Test")
    ...
    print(f"Quantized ResNeXt-101 (32x8D) DSP Test (mode: {args.dsp_mode})")
    results = _run_test(
