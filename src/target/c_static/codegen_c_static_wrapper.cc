...
void WrapperGenerator::EmitDSPWrappers(
  ...
  // Forward declaration for TIDL bridge init function (defined in tidl_bridge.c).
  // Emitted at file scope so it is visible inside cg_main_dsp.
  if (tidl_runtime) {
    os << "extern \"C\" int32_t tidl_bridge_init_all(void);\n\n";
  }
  ...
  for (const auto& [func_name, func_info] : functions) {
    ...
    // Initialize all TIDL subgraph handles on first inference call.
    // tidl_bridge_init_all() is defined in tidl_bridge.c (generated by the TIDL
    // pipeline).  Returns 0 on success; -1 if any subgraph handle fails to init.
    if (tidl_runtime) {
      os << "  {\n";
      os << "    static int32_t _tidl_init_done = 0;\n";
      os << "    if (!_tidl_init_done) {\n";
      os << "      if (tidl_bridge_init_all() != 0) { return -1; }\n";
      os << "      _tidl_init_done = 1;\n";
      os << "    }\n";
      os << "  }\n\n";
    }
