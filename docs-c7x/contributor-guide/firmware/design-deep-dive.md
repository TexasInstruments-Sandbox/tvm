...
C7x DSP. The TVM C static lib backend produces a relocatable C7x ELF module
...
[Deploying Firmware](../../user-guide/deploying-firmware.md) and
...
`g_cg_main_dsp` is generated by `src/target/c_static_lib/`. Its signature is:
```c
...
1. **`tidl_bridge_cleanup()`** (if the loaded module exports this symbol) —
   frees TIDL subgraph instances so TIDL can release DMA channels, IALG
   memory, and MMA state before the module's code/data sections are freed.
   Looked up via `dyn_loader_query_symbol()`; a no-op when the module has no
   TIDL subgraphs (e.g. MMALIB-only builds).

2. **`TVMDSPRegFileCleanup()`** — Iterates the static register file (in the
...
3. **`tvm_model_unload(model_id)`** — Frees the model slot in the `g_models[]`
...
4. **`TVMDSPConstantsCleanup()`** — Frees all constants memory pools allocated
   by `TVMDSPLoadConstants()` during the INFER setup phase, and resets the
   constants subsystem (`g_initialized = 0`).

5. **`dyn_loader_unload(handle)`** — Calls `tracked_free_all()` to free all ELF
   segments (`.text`, `.data`, `.bss`, `.rodata.weights`) from the TVM DDR heap.

6. **Clear state** — Reset `g_loaded_module_handle`, `g_cg_main_dsp`, and
   `g_embedded_model_id` to zero.

If steps 2-4 are skipped or performed after step 5: the register file memory is
...
Skipping step 1 on a module with TIDL subgraphs leaks DMA channels and IALG memory
instead.
...
This firmware is the runtime target for TVM's C static lib backend when compiling
for C7x DSP. The workflow is:

1. **TVM compilation** (on development host): Compile a neural network model
   using the TVM C static lib backend, producing `lib0.c` and `weights.bin`
